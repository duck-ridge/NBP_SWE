...
@export var ini_pos = Vector2(582, 482)
...
func release_grab():
	...
	if position.distance_to(PlaceHolder1.position) < 50:
		get_node("../../PlaceHolder1").take_prove(2_2)
		hide()
	if position.distance_to(PlaceHolder2.position) < 50:
		get_node("../..").is_PL2_correct = false
		get_node("../../PlaceHolder2").take_prove(2_2)
		hide()
	if position.distance_to(PlaceHolder3.position) < 50:
		get_node("../../PlaceHolder3").take_prove(2_2)
		hide()
	else:
		position = ini_pos
