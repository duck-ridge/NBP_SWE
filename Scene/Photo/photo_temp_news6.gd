...
func _input(event):
	if event.is_action_pressed("ui_accept"):
		...
		for node in $NewsPhoto.get_children():
			...
			tween.tween_property(node, "scale", Vector2.ONE, 0.2).from(Vector2.ONE * 1.5).set_ease(Tween.EASE_OUT)
...
func basic_setting():
	...
	pro_pic.scale = Vector2(1, 1)
	con_pic.scale = Vector2(1, 1)
...
func slide_select_pic(standpoint:String):
	match standpoint:
		"pro":
			...
			tween.tween_property(pro_pic, "modulate:a", 1.0, 0.5).from(0.0)
			tween.tween_property(pro_pic, "scale", Vector2(0.8, 0.8), 0.5).from(Vector2(1, 1))
			tween.parallel().tween_property(pro_pic, "position", Vector2(380, -330) - news_photo_envolop.position, 1.5).from(Vector2(-35, 55) + Vector2(0, -70)).set_trans(Tween.TRANS_EXPO)
			tween.parallel().tween_property(pro_pic, "rotation_degrees", 8, 1.5)
		...
		"con":
			print("con")
			print(news_photo_envolop.position)
			rect_for_pic(standpoint)
			con_pic.show()
			var tween = create_tween()
			tween.set_trans(Tween.TRANS_SINE)
			
			tween.tween_property(con_pic, "modulate:a", 1.0, 0.5).from(0.0)
			tween.tween_property(con_pic, "scale", Vector2(0.8, 0.8), 0.5).from(Vector2(1, 1))
			tween.parallel().tween_property(con_pic, "position", Vector2(-380, -330) - news_photo_envolop.position, 1.5).from(Vector2(-75, -51) + Vector2(0, -70)).set_trans(Tween.TRANS_EXPO)
			tween.parallel().tween_property(con_pic, "rotation_degrees", -4, 1.5)
