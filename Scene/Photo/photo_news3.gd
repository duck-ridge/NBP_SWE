...
func basic_setting():
	...
	pro_pic.scale = Vector2.ONE
	con_pic.scale = Vector2.ONE
...
func print_selected_node(nodeArray):
	for node in nodeArray:
		var tween = create_tween()
		tween.tween_property(node, "scale", Vector2.ONE * 1.2, 0.2).from(Vector2.ONE).set_ease(Tween.EASE_OUT)
		tween.parallel().tween_property(node, "modulate:a", 0, 0.2).from(1.0)
...
func slide_select_pic(standpoint:String):
	match standpoint:
		"pro":
			...
			tween.tween_property(pro_pic, "scale", Vector2(0.8, 0.8), 0.5).from(Vector2.ONE)
			tween.parallel().tween_property(pro_pic, "position", Vector2(300, -260) - news_photo_envolop.position, 1.5).from(Vector2(23, -83)).set_trans(Tween.TRANS_EXPO)
			tween.parallel().tween_property(pro_pic, "rotation_degrees", 15, 1.5).from(0).set_trans
		...
		"con":
			...
			tween.tween_property(con_pic, "scale", Vector2(0.8, 0.8), 0.5).from(Vector2.ONE)
			tween.parallel().tween_property(con_pic, "position", Vector2(-300, -260) - news_photo_envolop.position, 1.5).from(Vector2(-74, -98)).set_trans(Tween.TRANS_EXPO)
			tween.parallel().tween_property(con_pic, "rotation_degrees", -5, 1.5).from(0)
			con_pic.z_index = 1
...
func _on_photo_pro_area_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.button_index == 1 and event.is_pressed():
			Global.emit_signal("filled_newspaper", 5)
			
			
func _on_photo_con_area_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.button_index == 1 and event.is_pressed():
			Global.emit_signal("filled_newspaper", 6)
